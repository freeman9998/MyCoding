...
public class LifeCycleCounterServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		super.init(config);
		System.out.println("1.init() 메쏘드-->생성자 호출직후에 단한번호출[객체초기화,리소스획득]");
	...
	public void destroy() {
		super.destroy();
		System.out.println("3.destroy() 메쏘드-->객체가메모리에서 해제될때호출[리소스해지]");
	}
	private int count = 0;
	protected void service(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		  System.out.println("2.service()메쏘드--> 클라이언트요청시마다호출-->요청IP "+request.getRemoteAddr()+"==>"+this);
