...
SELECT NAME,SUBSTR(JUMIN,3,4) AS BIRTHDAY FROM STUDENT WHERE DEPTNO1='101';
   SELECT NAME,CONCAT(SUBSTR(JUMIN,3,2),'월')||CONCAT(SUBSTR(JUMIN,5,2),'일')
   AS BIRTHDAY FROM STUDENT 
   WHERE DEPTNO1='101';
   SELECT * FROM STUDENT;
<< 결과1 >>
-----------------------------------
...
4.Student 테이블에서 deptno1 이 101번인 학과 학생들의 이름과
  ...
  번호를 * 로 표시해서 출력하세요
  SELECT NAME,TEL,REPLACE(TEL,SUBSTR(TEL,8),'-****') "REPLACE" 
FROM STUDENT 
WHERE DEPTNO1='101';
-----------------------------------------------------------------
 NAME		  TEL	            	REPLACE
