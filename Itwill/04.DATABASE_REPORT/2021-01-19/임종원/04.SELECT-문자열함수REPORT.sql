...
select name,substr(jumin,3,4)birthday from student where deptno1 like '1_1'; 
select name, substr(jumin,3,2)||'월'||substr(jumin,5,2)||'일'birthdaty from student where deptno1 = 101;
...
select name , tel , substr(tel,1,instr(tel,')',3)-1)"AREA CODE" from student  where deptno1 =201;
...
select name , tel ,REPLACE(tel,substr(tel,8),'-****')"REAPLACE" from student where deptno1 = 101;
