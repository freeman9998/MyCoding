1. Student 테이블에서 jumin 컬럼을 이용해서 1전공이
   ...
   SELECT NAME, CONCAT(CONCAT(SUBSTR(JUMIN,3,2),'월'),
   CONCAT(SUBSTR(JUMIN,5,2),'일')) "Birthday" FROM STUDENT WHERE DEPTNO1=101;
...
3. Studnt 테이블에서 전공1(deptno1) 이 101번인 학생들의 이름과
   주민등록번호를 아래와같이 출력하세요.
   SELECT NAME,JUMIN,REPLACE(JUMIN,SUBSTR(JUMIN,7),'*******') "REPLACE" FROM STUDENT;
   SELECT NAME,JUMIN,REPLACE(JUMIN,SUBSTR(JUMIN,7),'-*******') "REPLACE" FROM STUDENT;
...
4.Student 테이블에서 deptno1 이 101번인 학과 학생들의 이름과
  ...
  번호를 * 로 표시해서 출력하세요
SELECT NAME,TEL,REPLACE(TEL,SUBSTR(TEL,INSTR(TEL,'-')),'-****') "REPLACE"
