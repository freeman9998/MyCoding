...
SELECT EMP_NAME, SALARY/18+100000 AS "100000+SAL", SALARY*2/18+200000 AS "200000+SAL" FROM TEMP;
...
4) TEMP 에서 성명을 보여줄 때 직급을 괄호 안에 함께 보여주는 SQL을 작성하시오.

SELECT EMP_NAME||'('||LEV||')' AS "성명" FROM TEMP;
SELECT EMP_NAME||''''||LEV||'''' AS "성명" FROM TEMP;

5) TEMP 테이블에서 HOBBY(취미) 가 NULL 이 아닌 사람의 성명을 읽어오는 SQL을 작성하시오.

SELECT EMP_NAME FROM TEMP WHERE HOBBY IS NOT NULL;

6) TEMP테이블에서 성명과 생년월일을 작은따옴표로 묶어서 하나의 문자열로 보여주는 SQL을 작성하시오.

SELECT ''''||EMP_NAME||BIRTH_DATE||'''' AS "NAME_BIRTH" FROM TEMP;

7)EMP 테이블을 이용해서 취미가 있는 사람 중
  직급이 과장인 사람의 사번, 성명, 취미를 읽어오는 SQL을 작성 하시오.
