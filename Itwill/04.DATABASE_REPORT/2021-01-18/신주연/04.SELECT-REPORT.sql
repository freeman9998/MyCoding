...
SELECT EMP_NAME ,SALARY/18,SALARY*2/18 FROM TEMP;SELECT EMP_NAME ,SALARY/18,SALARY*2/18 FROM TEMP;

2) 위에서 구한 월 급여에 교통비가 10만원씩 지급된다면 (짝수 달은20만원) 위의 문장이 어떻게 바뀔 지
   작성해 보세요.

SELECT EMP_NAME, SALARY/18+100000 "100000+SAL",SALARY/18+200000 "200000+SAL" FROM TEMP;SELECT EMP_NAME, SALARY+100000,SALARY+200000 FROM TEMP;

3)TEMP 의 자료 중 EMP_ID 와 EMP_NAME 을 각각 ‘사번’,‘성명’
  으로 표시되어DISPLAY 되도록 COLUMN ALIAS를 부여하여 SELECT 하시오.

SELECT EMP_ID "사번" ,EMP_NAME "성명" FROM TEMP;SELECT EMP_ID "사번" ,EMP_NAME "성명" FROM TEMP;

4) TEMP 에서 성명을 보여줄 때 직급을 괄호 안에 함께 보여주는 SQL을 작성하시요.

 SELECT EMP_NAME||'('||LEV||')' "성명" FROM TEMP;

SELECT EMP_NAME||''''||LEV||'''' "성명" FROM TEMP;SELECT EMP_NAME||'('||LEV||')' "성명" FROM TEMP;
