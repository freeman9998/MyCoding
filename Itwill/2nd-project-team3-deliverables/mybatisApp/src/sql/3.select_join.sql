...
on ce.course_id=c.course_id where s.stud_id=7;
...
course + tutor join
*/
SELECT course_id, c.name, description, start_date, end_date,t.tutor_id, t.name as tutor_name, email
      FROM tutors t 
      left outer join courses c 
      on t.tutor_id=c.tutor_id
      where c.course_id=3;
