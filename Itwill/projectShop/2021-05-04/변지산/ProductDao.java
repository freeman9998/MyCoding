...
public class ProductDao {
	public Product selectProduct(int no) throws Exception{
		String driverClass="oracle.jdbc.OracleDriver";
		String url="jdbc:oracle:thin:@182.237.126.19:1521:xe";
		String user="javabackend11";
		String password="javabackend11";
		
		String selectProOneSql="select p_no, p_name, p_price, p_image, p_desc, p_click_count from product where p_no = "+no;
		...
		while(rs.next()) {
			...
			product = new Product(p_no, p_name, p_price, p_img, p_desc, p_count);
		}
		rs.close();
		stmt.close();
		con.close();
	...
	public ArrayList<Product> selectAllProduct() throws Exception{
		...
		while(rs.next()) {
			...
			productList.add(new Product(p_no, p_name, p_price, p_img, p_desc, p_count));
		}
		rs.close();
		stmt.close();
		con.close();
