...
<%!public String getTitleString(ProductInquiry productinquiry) {
		StringBuilder title = new StringBuilder(128);
		String t = productinquiry.getB_title();
		if (t.length() > 20) {
...
<%
	...
	//페이징(계산)을위한DTO,VO
	InquiryPageInputDto InquirypageInputDto=
			new InquiryPageInputDto(rowCountPerPage, pageCountPerPage, pageno, "", "");
	
	//게시물조회	
	InquiryListPageDto inquiryListPage 
		= BoardService.getInstance().findQnaList(InquirypageInputDto, sMemberId);
            ...
            <div class="col-md-2 col-sm-1 hidden-xs display-table-cell v-align box" id="navigation" style="height: 94%;">
                <div class="navi">
                    <ul>
                        ...
                        <li class="active"><a href="mypage_inquiry_form.jsp"><i class="fa fa-tasks" aria-hidden="true"></i><span class="hidden-xs hidden-sm">상품문의</span></a></li>
            ...
            <div class="user-dashboard" style="width:100%;">
                    ...
                    <div class="row" style="width:70%;margin:0 0 3% 26%;">
                        <div class="col-md-5 col-sm-5 col-xs-12 gutter"style="width:91%;">
                            ...
                            <div class="sales">
                                <h2 class="my_title">상품문의</h2>
                                <form name="f" method="post">
			
							<table border="1">
								<tr>
									<td width=300 align=center bgcolor="0E1A35" height="25" style="color:white; font-size:17px">상품정보</td>
									<td width=300 align=center bgcolor="0E1A35" style="color:white; font-size:17px">문의내용</td>
								...
								<!-- loop start -->
								<%for(ProductInquiry inquiry: inquiryListPage.getList()){ %>
								<tr>
									
									<td width=300 align=center bgcolor="ffffff" style="padding-left: 10">
										<a href="product_detail_form.jsp?p_no=<%=inquiry.getP_no()%>" class="user">
										<%=inquiry.getP_name()%></a>
									<td width=300 align=center bgcolor="ffffff" style="padding-left: 10">
										<a href="mypage_inquiry_view.jsp?boardno=<%=inquiry.getB_no()%>" class="user">
										<%=getTitleString(inquiry)%></a>
									<td width=150 align=center bgcolor="ffffff"><%=inquiry.getM_id()%></td>
									<td width=150 align=center bgcolor="ffffff"><%=inquiry.getB_date()%></td>
								</tr>
								<%} %>
								<!--loop end  -->
							...
							<table>							
								<tr>
									<td align="center">
										<%if (inquiryListPage.isShowFirst()) {%> 
											<a href="mypage_inquiry_form.jsp?pageno=1">◀◀</a>&nbsp; 
										<%}%> 
										<%if (inquiryListPage.isShowPreviousGroup()) {%>
											<a href=".mypage_inquiry_form.jsp?pageno=<%=inquiryListPage.getPreviousGroupStartPageNo()%>">◀</a>&nbsp;&nbsp;
										<%}%>
										<%
										 	for (int i = inquiryListPage.getStartPageNo(); i <= inquiryListPage
										 			.getEndPageNo(); i++) {
										 	if (inquiryListPage.getSelectPageNo() == i) {
										%>
										 <font color='red'><strong><%=i%></strong></font>&nbsp;
										<%} else {%>
										<a href="mypage_inquiry_form.jsp?pageno=<%=i%>"><strong><%=i%></strong></a>&nbsp;
										<%
										   }
										  }%>
										<%if (inquiryListPage.isShowNextGroup()) {%> 
											<a href="mypage_inquiry_form.jsp?pageno=<%=inquiryListPage.getNextGroupStartPageNo()%>">▶&nbsp;</a>
										<%}%> 
										<%if (inquiryListPage.isShowLast()) {%> 
											<a href="mypage_inquiry_form.jsp?pageno=<%=inquiryListPage.getTotalPageCount()%>">▶▶</a>&nbsp;
										<%}%>
									</td>
								</tr>
							</table>
