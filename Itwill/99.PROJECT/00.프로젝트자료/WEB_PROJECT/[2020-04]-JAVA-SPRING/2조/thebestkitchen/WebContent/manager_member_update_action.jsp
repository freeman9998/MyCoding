...
<%
	if(request.getMethod().equalsIgnoreCase("GET")){
		response.sendRedirect("manager_member_form.jsp");
		return;
	}
	try{
		...
		for(int i=0;i<checkbox.length;i++){
			...
			memberService.update(new Member(memberId,password,name,phone,retire,admin,address,daddress,postcode,birth));
		...
		e.printStackTrace();
		RequestDispatcher rd = request.getRequestDispatcher("manager_member_form.jsp");
