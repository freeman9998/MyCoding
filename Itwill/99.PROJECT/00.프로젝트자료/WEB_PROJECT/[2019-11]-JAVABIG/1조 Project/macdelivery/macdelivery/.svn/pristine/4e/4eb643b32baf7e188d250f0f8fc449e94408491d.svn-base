...
public class MemberAddressListDao {
	...
	public int update(MemberAddressList memberAddressList) throws Exception {
		...
		try {
			...
			pstmt.setInt(4, memberAddressList.getM_no());
			pstmt.setInt(5, memberAddressList.getMa_no());
			int rows = pstmt.executeUpdate();
			return rows;
		} finally {
			if (pstmt != null)
	...
	public ArrayList<MemberAddressList> findAddressByNo(int m_no) throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String selectQuery = "select * from MemberAddressList where m_no=? order by ma_no asc";
		try {
			con = dataSource.getConnection();
			pstmt = con.prepareStatement(selectQuery);
			pstmt.setInt(1, m_no);
	...
	public int delete(int m_no) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		String deleteQuery = "delete from MemberAddressList where m_no=?";
		try {
			con = dataSource.getConnection();
			pstmt = con.prepareStatement(deleteQuery);
			pstmt.setInt(1, m_no);
