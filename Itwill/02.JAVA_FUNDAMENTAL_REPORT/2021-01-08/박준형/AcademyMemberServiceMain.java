...
public class AcademyMemberServiceMain {
	...
	public static void main(String[] args) {
		...
		AcademyMember[] findStaffGangsa = academyMemberService.findAcademyGangsaBySubject("자바");
		...
		findStaffDepart=academyMemberService.findAcademyMembers(new AcademyStaff(0, null, "생산"));
		for (AcademyMember academyMember : findStaffDepart) {
			academyMember.print();
		}
		System.out.println("5.AcademyMember 중에서 자바과목인 AcademyGangsa 객체들 배열참조변수반환해줘");
		findStaffGangsa=academyMemberService.findAcademyMembers(new AcademyGangsa(0, null, "DB"));
		for (AcademyMember academyMember : findStaffGangsa) {
			academyMember.print();
		}
	}
