/*
		 * 성적관리에서 학생2명 데이타를 담을 변수선언
		 * 	- 번호, 이름, 국어, 영어, 수학, 총점, 평균, 평점(A,B,C), 석차
		 */
		
		/*
		 * 학생 2명의 기본데이터(번호, 이름, 국어, 영어, 수학) 입력 
		 */
		
		/*
		 * 학생2명의 국어, 영어, 수학점수의 유효성체크
		 *   - 100점이 넘는 수나 음수가 입력되면 메세지를 출력하고 return(프로그램종료)한다.
		 */

		
		/*
		 * 학생 2명의 기본데이터(번호, 이름, 국어, 영어, 수학)를 이용해서
		 * 총점, 평균, 평점, 계산 후 대입 (석차는 0을 대입)
		 * 평점(A,B,C,D,F)계산시 switch문을 사용해보세요
		 */
		
		
		
		
		
		/*
		- 출력포맷
 		- 평균은 소수점이하 2째자리수에서 반올림하세요(%f)
 		- 석차는 구하지마세요
		--------------학생 성적출력-------------------
		학번  이름   국어 영어 수학 총점 평균 평점 석차
		-----------------------------------------------
		 1   김경호   42   56   78   334  34.8  A   3  
		 2   김경수   45   53   76   334  34.8  A   2  
		-----------------------------------------------
		*/
public class StudentScorePrintSwitch {

	public static void main(String[] args) {
		//학생2명의 변수선언과 기본데이타 입력
		int no = 1;
		int no1 = 2;
		String name = "김경호";
		String name1 = "김경수";
		int kor = 70;
		int kor1 = 45;
		int eng = 68;
		int eng1 = 53;
		int math = 55;
		int math1 = 76;
		//학생2명의 총점,평균,평점,석차 변수선언
		int tot, tot1;
		double avg, avg1;
		char grade, grade1;
		int rank, rank1;
		// 학생2명의 국어, 영어, 수학점수의 유효성체크
		if (kor < 0 || kor > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		} else if (eng < 0 || eng > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		} else if (math < 0 || math > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		}

		if (kor1 < 0 || kor1 > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		} else if (eng1 < 0 || eng1 > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		} else if (math1 < 0 || math1 > 100) {
			System.out.println("점수가 유효하지 않습니다.");
			return;
		}
		//학생 2명의 기본데이터를 이용해서 총점, 평균, 평점, 계산 후 대입 (석차는 0을 대입)
		tot = kor + eng + math;
		tot1 = kor1 + eng1 + math1;
		avg = tot / 3.0;
		avg1 = tot1 / 3.0;
		rank = 0;
		rank1 = 0;
		//평점(A,B,C,D,F)계산시 switch문을 사용해보세요
		switch ((int) avg / 10) {
		case 10:
		case 9:
			grade = 'A';
			break;
		case 8:
			grade = 'B';
			break;
		case 7:
		case 6:
			grade = 'C';
			break;
		case 5:
			grade = 'D';
			break;
		default:
			grade = 'F';
			break;
		}
		switch ((int) avg1 / 10) {
		case 10:
		case 9:
			grade1 = 'A';
			break;
		case 8:
		case 7:
			grade1 = 'B';
			break;
		case 6:
			grade1 = 'C';
			break;
		case 5:
			grade1 = 'D';
			break;
		default:
			grade1 = 'F';
			break;
		}
		System.out.printf("----------------학생 성적출력------------------%n");
		System.out.printf("%s %s %s %s %s %s %s %s %s %n", "학번", "이름", "국어", "영어", "수학", "총점", "평균", "평점", "석차");
		System.out.printf("-----------------------------------------------%n");
		System.out.printf("%2d %4s %d %4d %4d %4d %5.1f %3c %4d %n", no, name, kor, eng, math, tot, avg, grade, rank);
		System.out.printf("%2d %4s %d %4d %4d %4d %5.1f %3c %4d %n", no1, name1, kor1, eng1, math1, tot1, avg1, grade1,rank1);
		System.out.print("-----------------------------------------------");
		
		

	}

	}
