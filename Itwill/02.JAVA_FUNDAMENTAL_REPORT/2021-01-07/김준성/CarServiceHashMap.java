...
public class CarServiceHashMap {
	 ...
	 */
	public boolean carIn(Car carIn) {
		boolean isSuccess = false;
		if (count - carMap.size() < 0) {
			return isSuccess;
		}

		carMap.put(carIn.getNo(), carIn);
		isSuccess = true;
		return isSuccess;
	}
	...
	public void print() {
		...
		Iterator<String> carKeySetIter = carKeySet.iterator();
		while (carKeySetIter.hasNext()) {
			carMap.get(carKeySetIter.next()).print();
	...
	public ArrayList<Car> findByInTime(int inTime) {
		...
		Iterator<String> carKeySetIter = carKeySet.iterator();
		while (carKeySetIter.hasNext()) {
			Car tempCar = carMap.get(carKeySetIter.next());
