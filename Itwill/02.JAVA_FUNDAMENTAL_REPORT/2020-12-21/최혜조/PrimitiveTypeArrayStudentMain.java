...
public class PrimitiveTypeArrayStudentMain {
	...
	public static void main(String[] args) {
		...
		String[] nameArray = {"KIM","LEE","PARK","CHOI","SIM","GOO","PIM","MIN","ALM","LIM"};
		int[] kora = {100,56,12,89,91,67,88,93,67,99};
		int[] enga = {80,57,21,83,93,76,81,67,12,39};
		 ...
		 * 3번학생 찾아서 1명 정보출력
		 */
		System.out.println(" >> 3번학생 찾아서 1명 정보 출력(학생정보 중복되지않는다.)");
		for (int i = 0; i < noArray.length; i++) {
			if(noArray[i]==3) {
		 ...
		 * F학점 학생 찾아서 여러명 정보출력
		 */
		System.out.println(" >> F학점 학생 모두 찾아서 정보출력");
		for (int i = 0; i < noArray.length; i++) {
			if(gradea[i]=='F') {
				System.out.printf("%3d %5s %4d %4d %4d %4d %5.1f %3c %4d%n",
						noArray[i],nameArray[i],kora[i],enga[i],matha[i],tota[i],avga[i],gradea[i],ranka[i]);				
