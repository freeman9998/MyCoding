...
public class CarServiceMain {
	public static void main(String[] args) {
		...
		carService.ipChar(new Car("4521",8 ));
		carService.ipChar(new Car("4567",9 ));
		carService.ipChar(new Car("7891",9 ));
		
		System.out.println("1.CarService객체야 전체차량출력해줘");
		carService.print();
		
		System.out.println("2.CarService객체야 전체주차구획수반환해줘");
		int totPark = carService.totalNumberOfParkingDivisions();
		System.out.println(totPark);
		
		System.out.println("3.CarService객체야 주차가능주차구획수반환해줘");
		int totPossiblePark = carService.totalNumberOfPossibleParkingDivisions();
		System.out.println(totPossiblePark);
		
		System.out.println("5.CarService객체야 차량번호(4567번) 인자로줄께  차객체한대 참조변수반환해줘");
		Car noCar = carService.findByNo("4567");
		if(noCar != null) {
			noCar.print();
		}
		
		System.out.println("6.CarService객체야 입차시간(8시이후) 인자로줄께 차객체배열 참조변수반환해줘");
		Car[] inTimeCars = carService.findByInTimeCars(8);
		for (int i = 0; i < inTimeCars.length; i++) {
			inTimeCars[i].print();
		}
		
		System.out.println("7.CarService객체야 차량번호(7891번) 출차시간(12시)인자로줄께 출차시켜줘");
		carService.exitOnCarByNo("7891", 12);
