...
public class StudentService {
	...
	public Student[] findByGrade(char grade) {
		...
		findGrade = new Student[count];

		int index = 0;
		for (int i = 0; i < students.length; i++) {
			findGrade[index] = students[i];
			index++;
		}
	...
	public Student[] findByName(String name) {
		...
		int index = 0;
		Student[] findName = new Student[count];
		for (int i = 0; i < students.length; i++) {
		    if (students[i].getName().equals(name)) {
				findName[index] = students[i];
				index++;
			}
		}
		return findName;
		...
		for (int i = 0; i < students.length - 1; i++) {
			for (int j = 0; j < students.length - 1; j++)
				if (students[j].getRank() > students[j + 1].getRank()) {
					Student tempRank = students[j];
					students[j] = students[j + 1];
					students[j + 1] = tempRank;
