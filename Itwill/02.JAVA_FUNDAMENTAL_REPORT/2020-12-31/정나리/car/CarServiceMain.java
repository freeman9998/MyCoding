...
public class CarServiceMain {
	public static void main(String[] args) {
		...
		carService.ipChar(new Car("7891",7 ));
		...
		int cnt = carService.allParkingLot();
		System.out.println("전체 주차구획수 : " + cnt);
		
		System.out.println("3.CarService객체야 주차가능주차구획수반환해줘");
		cnt = carService.emptyParkingLot();
		System.out.println("주차가능구획수 : " + cnt);
		
		System.out.println("5.CarService객체야 차량번호(4567번) 인자로줄께  차객체한대 참조변수반환해줘");
		Car car = carService.findCarNo("4567");
		if(car != null) {
			car.print();
		}else {
		...
		Car[] car1 = carService.findCarInTime(8);
