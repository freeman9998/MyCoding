...
public class AccountServiceArrayList {
	...
	public int totBalance(){
		int totBalance=0;
		for (int i = 0; i < accountList.size(); i++) {
			Account totBalance1 = accountList.get(i);
			totBalance+=totBalance1.getBalance();
		}
		return totBalance;
	}
	/*
	...
	public ArrayList<Account> findByBalance(int balance) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount = accountList.get(i);
			if (tempAccount!=null&&tempAccount.getBalance()>balance) {
				count++;
			}
		}
		ArrayList<Account> findAccountList=new ArrayList<Account>(count);
		count = 0;
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount = accountList.get(i);
			if (tempAccount!=null&&tempAccount.getBalance()>balance) {
	...
	*/
	public ArrayList<Account> findByName(String owner) {
		...
		for (int i = 0; i < accountList.size(); i++) {
			Account tempAccount = accountList.get(i);
			if (tempAccount!=null&&tempAccount.getOwner().equals(owner)) {
				count++;
			}
		}
	...
	*/
		...
		for (int i = 0; i < accountList.size()-1; i++) {
			for (int j = 0; j < accountList.size()-1; j++) {
				if (accountList.get(j).getBalance() > accountList.get(j+1).getBalance()){
					Account tempUp = accountList.get(j);
					accountList.set(j,accountList.get(j+1));
					accountList.set(j+1,tempUp);
				}
			}
		}
	}
	...
	public void sortByBalanceDesc() {
		for (int i = 0; i < accountList.size()-1; i++) {
			for (int j = 0; j < accountList.size()-1; j++) {
				if (accountList.get(j).getBalance() < accountList.get(j+1).getBalance()){
					Account tempDown = accountList.get(j);
					accountList.set(j,accountList.get(j+1));
					accountList.set(j+1,tempUp);
				}
			}
	...
	public void updateAccount(Account updateAccount) {
		for (int i = 0; i < accountList.size(); i++) {
			if (accountList.get(i).getNo()==updateAccount.getNo()) {
				accountList.get(i).setAccountData(updateAccount.getNo(),
				updateAccount.getOwner(),updateAccount.getBalance(),updateAccount.getIyul()); 
			}
		}
	}
