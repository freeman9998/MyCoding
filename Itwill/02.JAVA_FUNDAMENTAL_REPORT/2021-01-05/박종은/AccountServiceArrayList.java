...
public class AccountServiceArrayList {
	...
	*/
	...
	public void ipGum(int no,int m) {
		Account accountIpGum = this.findByNo(no); 
		accountIpGum.deposit(m);
	}
		
		
	/*	Account accountIpGum = null; 
		for (int i = 0; i < accountList.size(); i++) {
		accountIpGum = accountList.get(i);
		if (accountIpGum.getNo() == no) {
			accountIpGum.deposit(m);
			accountList.set(i, accountIpGum);
			break;
		}
	...
	public void chulGum(int no,int m) {
		Account accountChulGum = this.findByNo(no);
		accountChulGum.withDraw(m);
	}
		/*
		Account accountChulGum = null;
		for (int i = 0; i < accountList.size(); i++) {
			accountChulGum = accountList.get(i);
			if (accountChulGum.getNo() == no) {
				accountChulGum.withDraw(m);
				accountList.set(i, accountChulGum);
				break;
			}
		}
	}
