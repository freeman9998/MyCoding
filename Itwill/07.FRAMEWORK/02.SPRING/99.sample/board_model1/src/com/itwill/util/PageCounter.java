...
public class PageCounter{	
	public static ListResultBean getPagingInfo(	
			int sPage,		//요청페이지 8
			int rCount,		//한화면당보여지는행의수 10
			int pCount,		//page index숫자 5
			int recordCount	//전체레코드수 598
			){
		...
		*/
		...
		if (recordCount > 1){ //1개 이상의 레코드가 존재하면...
			...
			if (modula != 0){//나머지가 1,2, 3, 4, 5,...., 9이면
				modula = 1;
		...
		//요청하는 페이지수가 전체페이지수보다큰경우
		if(sPage>totalPageCount){
			sPage=totalPageCount;
		}
...
//	 페이지에 [처음으로] 아이콘 출력여부를 판단하는 메소드
	  ...
	  public static void main(String[] args) {
		  ListResultBean bean = PageCounter.getPagingInfo(3, 10, 5, 598);
		  System.out.println("총페이지수:"+bean.getPageCount());
		  System.out.println("요청페이지:"+bean.getSelectPageNo());
		  System.out.println("페이지인덱스시작번호:"+bean.getStartPageNo());
		  System.out.println("페이지인덱스끝읕번호:"+bean.getEndPageNo());
		  System.out.println("이전페이지아이콘출력여부:"+bean.isShowPreviousGroup());
		  System.out.println("이후페이지아이콘출력여부:"+bean.isShowNextGroup());
