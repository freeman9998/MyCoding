...
public class CartDao {
	...
	public ArrayList<String> findAllPtitle(String m_id) throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String selectQuery = CartSQL.FIND_ALL_PTITLE;
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				titleList.add(rs.getString("p_title"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public boolean existedCart(String m_id, int p_no) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String existedQuery = "select count(*) cnt from cart where m_id = ? and p_no = ?";
		try {
			con = dataSource.getConnection();
