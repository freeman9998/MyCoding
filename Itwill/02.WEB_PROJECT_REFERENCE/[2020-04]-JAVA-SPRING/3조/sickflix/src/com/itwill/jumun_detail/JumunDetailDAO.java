...
private DataSource dataSource;
	...
	public int create(JumunDetail jumunDetail) throws Exception {
		...
		String insertQuery = JumunDetailSQL.INSERT_JUMUNDETAIL;
		try {
	...
	public int update(JumunDetail jumunDetail) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		String updateQuery = JumunDetailSQL.UPDATE_JUMUNDETAIL;
		try {
			con = dataSource.getConnection();
	...
	public int remove(int jd_no) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		String removeQuery = JumunDetailSQL.DELETE_JUMUNDETAIL;
		try {
			con = dataSource.getConnection();
	...
	public JumunDetail findJumunDetail(int jd_no) throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String selectQuery = JumunDetailSQL.FIND_JUMUNDETIAL;
		try {
			con = dataSource.getConnection();
	...
	/*
	 * 특정 주문의 주문상세정보 전부 찾아오기
	 */
	public ArrayList<JumunDetail> findDetailFromJumun(int j_no) throws Exception {
		ArrayList<JumunDetail> jumunDetailList= new ArrayList<JumunDetail>();
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String selectQuery = JumunDetailSQL.FIND_DETAIL_BY_JUMUN;
		try {
			con = dataSource.getConnection();
			pstmt = con.prepareStatement(selectQuery);
			pstmt.setInt(1, j_no);
	...
	public boolean existedJumunDetail(int jd_no) throws Exception{
		...
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String existedQuery="select count(*) cnt from jumun_detail where jd_no=?";
		try{
			con=dataSource.getConnection();
