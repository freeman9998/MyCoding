...
public class StoreDao {
	...
	public ArrayList<Store> selectAll() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				storeList.add(new Store(rs.getInt("s_no"),rs.getString("s_name"),
						rs.getString("s_address_si"),rs.getString("s_address_gu"),rs.getString("s_address_detail")));
			}
	 ...
	 * READ BY NAME
	 */
	public Store selectSNOByName(String name) throws Exception {
		Store store = null;
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				store = new Store(rs.getInt("s_no"),rs.getString("s_name"),
						rs.getString("s_address_si"),rs.getString("s_address_gu"),rs.getString("s_address_detail"));
			}
		} finally {
			if (con != null)
